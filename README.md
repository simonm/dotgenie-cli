# dotgenie

A fast, simple dotfiles manager written in Go.

## Features

- **Fast parallel symlinking** - Links hundreds of dotfiles in milliseconds
- **Layered configuration** - `common/` → `workstation/` → `hosts/<hostname>/`
- **Direct symlinks** - Edit `~/.config/foo` and it edits your managed file
- **Package management** - Ansible handles package installation
- **Cross-platform** - Linux (Arch, Ubuntu, Debian) and macOS

## Installation

```bash
curl -fsSL https://raw.githubusercontent.com/simonm/dotgenie-cli/main/install.sh | bash
```

Or download from [releases](https://github.com/simonm/dotgenie-cli/releases).

## Quick Start

```bash
# Initialize with your dotfiles repo
dotgenie init https://github.com/you/dotfiles

# Apply dotfiles and install packages
dotgenie apply

# Check status
dotgenie status
```

## Commands

| Command | Description |
|---------|-------------|
| `init [repo]` | Clone dotfiles repo and configure |
| `apply` | Link dotfiles and install packages |
| `status` | Show status of managed dotfiles |
| `adopt <path>` | Import existing configs into management |
| `sync` | Sync with remote repository |

### Apply Options

```bash
dotgenie apply                    # Full apply
dotgenie apply --dotfiles-only    # Only link dotfiles
dotgenie apply --packages-only    # Only install packages
dotgenie apply --dry-run          # Preview changes
dotgenie apply --verbose          # Show all files
```

### Adopt Options

```bash
dotgenie adopt ~/.config/nvim                    # Adopt to common/
dotgenie adopt --scope workstation ~/.config/hypr  # Adopt to workstation/
dotgenie adopt --scope host ~/.config/monitors.xml # Adopt to hosts/<hostname>/
dotgenie adopt --copy-only ~/.bashrc             # Copy without symlinking
dotgenie adopt -y ~/.config/*                    # Skip confirmation
```

## Dotfiles Repository Structure

Your dotfiles repo should have this structure:

```
dotfiles/
├── dotfiles/
│   ├── common/           # All systems
│   │   └── .config/
│   │       └── nvim/
│   ├── workstation/      # Desktop/laptop only
│   │   └── .config/
│   │       └── hypr/
│   └── hosts/
│       └── myhost/       # Host-specific
│           └── .config/
├── packages/
│   ├── common.yml        # All systems
│   ├── workstation.yml   # GUI packages
│   └── arch.yml          # OS-specific
├── ansible/
│   └── playbook.yml
└── config.yml            # Generated by init
```

## Configuration

`config.yml` is created during `dotgenie init`:

```yaml
os: arch
system_type: workstation
hostname: myhost
auto_pull_before_apply: true
auto_commit_after_adopt: true
auto_push_after_adopt: false
```

## Building from Source

```bash
go build -o dotgenie ./cmd/dotgenie
```

## License

MIT
